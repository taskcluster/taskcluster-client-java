...
public class Auth extends TaskClusterRequestHandler {
	 ...
	 */
	public CallSummary<EmptyPayload, GetClientCredentialsResponse> getCredentials(String clientId)
	 ...
	 * 
	 * **Required scopes**, in addition the scopes listed above, the caller must
	 * also posses the all the scopes that is given to the client that is
	 * created.
	 *
	 * See http://docs.taskcluster.net/auth/api-docs/#createClient
	 */
	public CallSummary<GetClientCredentialsResponse1, GetClientResponse> createClient(String clientId,
			GetClientCredentialsResponse1 payload) throws APICallFailure {
	 ...
	 */
	public CallSummary<GetClientCredentialsResponse1, GetClientResponse> modifyClient(String clientId,
			GetClientCredentialsResponse1 payload) throws APICallFailure {
	 ...
	 */
	public CallSummary<EmptyPayload, AzureSharedAccessSignatureResponse> azureTableSAS(String account, String table)
			throws APICallFailure {
		return apiCall(null, "GET", "/azure/" + account + "/table/" + table + "/read-write",
				AzureSharedAccessSignatureResponse.class);
	}

	/**
	 * Get temporary AWS credentials for `read-write` or `read-only` access to a
	 * given `bucket` and `prefix` within that bucket. The `level` parameter can
	 * be `read-write` or `read-only` and determines which type of credentials
	 * is returned. Please note that the `level` parameter is required in the
	 * scope guarding access.
	 * 
	 ...
	 * allowed on S3, but we forbid it here to discourage bad behavior. Also
	 * note that if your `prefix` doesn't end in a slash `/` the STS credentials
	 * will not require one to be to inserted. This is mainly a concern when
	 * assigning scopes to users and doing this right will prevent poor
	 * behavior. After we often want the `prefix` to be a folder in a `/`
	 * delimited folder structure.
	 *
	 * See http://docs.taskcluster.net/auth/api-docs/#awsS3Credentials
	 */
	public CallSummary<EmptyPayload, AWSS3CredentialsResponse> awsS3Credentials(String level, String bucket,
			String prefix) throws APICallFailure {
		return apiCall(null, "GET", "/aws/s3/" + level + "/" + bucket + "/" + prefix + "",
				AWSS3CredentialsResponse.class);
	}

	/**
	 * Export all clients except the root client, as a JSON list. This list can
	 * be imported later using `importClients`.
	 *
	 ...
	 */
	public CallSummary<ExportedClients, ExportedClients[]> importClients(ExportedClients payload)
			throws APICallFailure {
