...
/**
 * The AWS Provisioner is responsible for provisioning instances on EC2 for use
 * in TaskCluster. The provisioner maintains a set of worker configurations
 * which can be managed with an API that is typically available at
 ...
 * utility factor information to decide which instance type in which region
 * would be the optimal configuration.
 * 
 * Each EC2 instance type will declare a capacity and utility factor. Capacity
 * is the number of tasks that a given machine is capable of running
 * concurrently. Utility factor is a relative measure of performance between two
 * instance types. We multiply the utility factor by the spot price to compare
 * instance types and regions when making the bidding choices.
 *
...
public class AwsProvisioner extends TaskClusterRequestHandler {
	 ...
	 *
	 * See
	 * http://docs.taskcluster.net/aws-provisioner/api-docs/#createWorkerType
	 */
	public CallSummary<CreateWorkerTypeRequest, GetWorkerTypeRequest> createWorkerType(String workerType,
			CreateWorkerTypeRequest payload) throws APICallFailure {
	 ...
	 */
	public CallSummary<CreateWorkerTypeRequest, GetWorkerTypeRequest> updateWorkerType(String workerType,
			CreateWorkerTypeRequest payload) throws APICallFailure {
	...
	/**
	 * Delete a WorkerType definition, submits requests to kill all instances
	 * and delete the KeyPair from all configured EC2 regions
	 *
	 * See
	 * http://docs.taskcluster.net/aws-provisioner/api-docs/#removeWorkerType
	 */
	...
	/**
	 * Insert a secret into the secret storage. This should not normally be done
	 * through this API, but is provided for testing and completeness
	 *
	 * See http://docs.taskcluster.net/aws-provisioner/api-docs/#createSecret
	 */
	public CallSummary<GetSecretRequest, EmptyPayload> createSecret(String token, GetSecretRequest payload)
	...
	/**
	 * An instance will report in by giving its instance id as well as its
	 * security token. The token is given and checked to ensure that it matches
	 * a real token that exists to ensure that random machines do not check in.
	 * We could generate a different token but that seems like overkill
	 *
	 * See http://docs.taskcluster.net/aws-provisioner/api-docs/#instanceStarted
	 */
	public CallSummary<EmptyPayload, EmptyPayload> instanceStarted(String instanceId, String token)
			throws APICallFailure {
		return apiCall(null, "GET", "/instance-started/" + instanceId + "/" + token + "", EmptyPayload.class);
	}

	/**
	 * Remove a secret. It is very important that the consumer of a secret
	 * delete the secret from storage before handing over control to another
	 * process or else it could read the HTTP UserData endpoint and use the
	 * getSecrete() api here to get the secrets
	 *
	...
	/**
	 * Return the EC2 LaunchSpecifications for all combinations of regions and
	 * instance types or a list of reasons why the launch specifications are not
	 * valid
	 * 
	 * **This API end-point is experimental and may be subject to change without
	 * warning.**
	 *
	...
	/**
	 * Return the state of a given workertype as stored by the provisioner. This
	 * state is stored as three lists: 1 for all instances, 1 for requests which
	 * show in the ec2 api and 1 list for those only tracked internally in the
	 * provisioner.
	 *
