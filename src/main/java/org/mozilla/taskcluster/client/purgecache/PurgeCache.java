 ...
 * `purge-cache.taskcluster.net`, is responsible for publishing a pulse message
 * for workers, so they can purge cache upon request.
 * 
 * This document describes the API end-point for publishing the pulse message.
 * This is mainly intended to be used by tools.
 *
...
public class PurgeCache extends TaskClusterRequestHandler {
	 ...
	 * `provisionerId` and `workerType` in the routing-key. Workers should be
	 * listening for this message and purge caches when they see it.
	 *
	 * See http://docs.taskcluster.net/services/purge-cache/#purgeCache
	 */
	public CallSummary<PurgeCacheRequest, EmptyPayload> purgeCache(String provisionerId, String workerType,
			PurgeCacheRequest payload) throws APICallFailure {
