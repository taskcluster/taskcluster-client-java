...
/**
 * The task-graph scheduler, typically available at `scheduler.taskcluster.net`,
 * is responsible for accepting task-graphs and scheduling tasks for evaluation
 * by the queue as their dependencies are satisfied.
 * 
 * This document describes API end-points offered by the task-graph scheduler.
 * These end-points targets the following audience: * Post-commit hooks, that
 * wants to submit task-graphs for testing, * End-users, who wants to execute a
 * set of dependent tasks, and * Tools, that wants to inspect the state of a
 * task-graph.
 *
...
public class Scheduler extends TaskClusterRequestHandler {
	 ...
	 * ```js { ... tasks: [ { taskId: "XgvL0qtSR92cIWpcwdGKCA", requires: [],
	 * ... }, { taskId: "73GsfK62QNKAk2Hg1EEZTQ", requires:
	 * ["XgvL0qtSR92cIWpcwdGKCA"], task: { payload: { env: { DEPENDS_ON:
	 * "XgvL0qtSR92cIWpcwdGKCA" } ... } ... }, ... } ] } ```
	 * 
	 * **The `schedulerId` property**, defaults to the `schedulerId` of this
	 * scheduler in production that is `"task-graph-scheduler"`. This property
	 * must be either undefined or set to `"task-graph-scheduler"`, otherwise
	 * the task-graph will be rejected.
	 * 
	 * **The `taskGroupId` property**, defaults to the `taskGraphId` of the
	 * task-graph submitted, and if provided much be the `taskGraphId` of the
	 * task-graph. Otherwise the task-graph will be rejected.
	 * 
	 * **Task-graph scopes**, a task-graph is assigned a set of scopes, just
	 * like tasks. Tasks within a task-graph cannot have scopes beyond those the
	 * task-graph has. The task-graph scheduler will execute all requests on
	 * behalf of a task-graph using the set of scopes assigned to the
	 ...
	 * required to define tasks for this `provisionerId` and `workerType`. See
	 * the queue for details on permissions required. Note, the task-graph does
	 * not require permissions to schedule the tasks. This is done with scopes
	 * provided by the task-graph scheduler.
	 * 
	 ...
	 * has a task-graph specific routing-key: `<route>`, then the poster will be
	 * required to posses the scope `scheduler:route:<route>`. And when the an
	 * AMQP message about the task-graph is published the message will be CC'ed
	 * with the routing-key: `route.<route>`. This is useful if you want another
	 * component to listen for completed tasks you have posted.
	 *
	 * See http://docs.taskcluster.net/scheduler/api-docs/#createTaskGraph
	 */
	public CallSummary<TaskGraphDefinition1, TaskGraphStatusResponse> createTaskGraph(String taskGraphId,
			TaskGraphDefinition1 payload) throws APICallFailure {
	 ...
	 * similar to the request format for creating task-graphs. But `routes` key,
	 * `scopes`, `metadata` and `tags` cannot be modified.
	 * 
	 * **Referencing required tasks**, just as when task-graphs are created,
	 * each task has a list of required tasks. It is possible to reference all
	 * `taskId`s within the task-graph.
	 * 
	 ...
	 */
	public CallSummary<TaskGraphDefinition, TaskGraphStatusResponse> extendTaskGraph(String taskGraphId,
			TaskGraphDefinition payload) throws APICallFailure {
	 ...
	 * structure_, along with `metadata` and `tags`, but not information about
	 * all tasks.
	 * 
	 ...
	 * later. Also note that note all of the information is formalized yet. The
	 * JSON schema will be updated to reflect formalized values, we think it's
	 * safe to consider the values stable.
	 * 
	 * Take these considerations into account when using the API end-point, as
	 * we do not promise it will remain fully backward compatible in the future.
	 *
	 ...
	 */
	public CallSummary<EmptyPayload, InspectTaskGraphTaskResponse> inspectTask(String taskGraphId, String taskId)
			throws APICallFailure {
		return apiCall(null, "GET", "/task-graph/" + taskGraphId + "/inspect/" + taskId + "",
				InspectTaskGraphTaskResponse.class);
	}
