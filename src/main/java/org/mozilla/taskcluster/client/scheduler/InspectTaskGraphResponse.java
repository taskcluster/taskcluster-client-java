...
/**
 * Information about a **task-graph** as known by the scheduler, with all the
 * state of all individual tasks.
 *
 * See
...
public class InspectTaskGraphResponse {
	...
	public class Tasks {
		...
		/**
		 * List of `taskId`s that requires this task to be _complete
		 * successfully_ before they can be scheduled.
		 */
		...
		/**
		 * List of `taskId`s that have yet to complete successfully, before this
		 * task can be scheduled.
		 */
		public String[] requiresLeft;

		/**
		 * Number of times to _rerun_ the task if it completed unsuccessfully.
		 * **Note**, this does not capture _retries_ due to infrastructure
		 * issues.
		 */
		public int reruns;
		...
		/**
		 * true, if the scheduler considers the task node as satisfied and hence
		 * no-longer prevents dependent tasks from running.
		 */
