...
public class TaskGraphDefinition1 {
	...
	/**
	 * List of nodes in the task-graph, each featuring a task definition and
	 * scheduling preferences, such as number of _reruns_ to attempt.
	 */
	public class Tasks {
		...
		/**
		 * Task identifier (`taskId`) for the task when submitted to the queue,
		 * also used in `requires` below. This must be formatted as a **slugid**
		 * that is a uuid encoded in url-safe base64 following [RFC 4648 sec.
		 * 5](http://tools.ietf.org/html/rfc4648#section-5)), but without `==`
		 * padding.
		 */
